...
export default function AdminCorporateCareersPage() {
  const { token } = useAuth();
  ...
  const fetchItems = useCallback(async () => {
    if (!token) return;
    try {
      const data = await apiClient('/api/corporate/careers', { token });
      setItems(Array.isArray(data) ? data : []);
    } catch {
      setMessage({ type: 'error', text: '채용 목록을 불러오지 못했습니다.' });
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await apiClient(`/api/corporate/careers/${editingId}`, {
          method: 'PUT',
          body: JSON.stringify(body),
          token: token!,
        });
        setMessage({ type: 'success', text: '채용 정보가 수정되었습니다.' });
      } else {
        await apiClient('/api/corporate/careers', {
          method: 'POST',
          body: JSON.stringify(body),
          token: token!,
        });
        setMessage({ type: 'success', text: '채용 정보가 추가되었습니다.' });
      }
      setModalOpen(false);
      await fetchItems();
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      await apiClient(`/api/corporate/careers/${deleteTarget.id}`, {
        method: 'DELETE',
        token: token!,
      });
      setMessage({ type: 'success', text: '채용 정보가 삭제되었습니다.' });
    ...
    <div className="space-y-6">
      ...
      >
        <p className="text-sm text-admin-text">
          <strong>{deleteTarget?.title}</strong> 채용 정보를 삭제하시겠습니까?
          <br />
