...
export default function AdminCorporateServicesPage() {
  const { token } = useAuth();
  ...
  const fetchServices = useCallback(async () => {
    if (!token) return;
    try {
      const data = await apiClient('/api/corporate/services', { token });
  ...
  const handleSave = async () => {
    if (!form.title.trim()) {
      setMessage({ type: 'error', text: '서비스명은 필수입니다.' });
      return;
    }
    ...
    try {
      if (editingId) {
        await apiClient(`/api/corporate/services/${editingId}`, {
          method: 'PUT',
        ...
        await apiClient('/api/corporate/services', {
          method: 'POST',
          body: JSON.stringify(body),
          token: token!,
        });
  ...
  const handleDelete = async () => {
    ...
    try {
      await apiClient(`/api/corporate/services/${deleteTarget.id}`, {
        method: 'DELETE',
        token: token!,
      });
  ...
  const columns = [
    ...
    { key: 'title', label: '서비스명' },
    { key: 'title_en', label: '영문명' },
  ...
  const tableData: Record<string, ReactNode>[] = services.map((item) => ({
    ...
    title_en: <span className="text-admin-text-secondary">{item.title_en || '-'}</span>,
    description: (
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Services Table */}
      <AdminCard
        title={`서비스 목록 (${services.length}개)`}
        actions={
