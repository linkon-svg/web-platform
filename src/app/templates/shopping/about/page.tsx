...
export default async function AboutPage() {
  const config = await fetchAPI('/api/shopping/config');

  const shopName = config?.shop_name || "SOLID";
  const aboutContent = config?.about_content || null;
  const promoMessages = config?.promo_text
    ? [config.promo_text]
    : ["전 상품 무료 배송 & 무료 반품"];

  const philosophy1 = aboutContent
    || "SOLID는 본질에 집중합니다. 불필요한 장식을 덜어내고, 순수한 형태와 최상의 소재만으로 완성되는 옷을 추구합니다. 우리의 디자인은 시간이 흘러도 변하지 않는 가치를 담고 있습니다.";
      ...
      <section className="px-6 lg:px-12 py-20 lg:py-28">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 mb-20 lg:mb-28">
          <div>
            ...
            >
              2010년 서울에서 시작된 {shopName}는 한국적 미니멀리즘과 유럽의 정통 테일러링을 결합하여
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 mb-20 lg:mb-28">
          ...
          <div className="lg:order-2">
            ...
            >
              모든 {shopName} 제품은 엄격하게 선별된 원단과 숙련된 장인의 손에서 탄생합니다.
